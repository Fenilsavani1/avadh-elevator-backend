  ...
  try {
    ...
    const paymentsByDay = await PaymentEntry.aggregate([
      ...
      },
      {
        $group: {
          _id: {
            $dateToString: { format: "%Y-%m-%d", date: { $add: ["$date", 1000 * 60 * 60 * 5.5] } } // adjust for IST example
          },
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      report.push({
        date: dayStr,
        revenue: revenueAmount, // keep total project cost here
        received,
        due: runningDue - received
      });

      runningDue -= received; // update carry forward
    }
